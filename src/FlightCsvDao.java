...
public class FlightCsvDao {
    ...
    private boolean saveAll(List<Flight> list) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(FILE))) {
            pw.println("id,code,origin,destination,depart_time,arrive_time,price,seats_total,seats_left");
            for (Flight f : list) {
                pw.printf("%d,%s,%s,%s,%s,%s,%.2f,%d,%d\n",
                        f.getId(), f.getCode(), f.getOrigin(), f.getDestination(),
                        f.getDepartTime().format(dtf), f.getArriveTime().format(dtf),
                        f.getPrice(), f.getSeatsTotal(), f.getSeatsLeft());
            }
            return true;
        } catch (IOException e) {
            e.printStackTrace();
